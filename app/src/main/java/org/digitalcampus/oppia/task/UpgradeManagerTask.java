...
public class UpgradeManagerTask extends AsyncTask<Void, String, BasicResult> {
    ...
    protected BasicResult doInBackground(Void... params) {
        ...
        if (!prefs.getBoolean("upgradeV91", false)) {
            upgradeV91();
            prefs.edit().putBoolean("upgradeV91", true).apply();
            publishProgress(this.ctx.getString(R.string.info_upgrading, "v91"));
        }
    ...
    /**
     * Convert reminder week days separated by commas in properties to a string set compatible with Preferences
     */
    private void upgradeV91() {
        String daysSerialized = ctx.getString(R.string.prefCoursesReminderDaysDefaultSerialized);
        if (daysSerialized.endsWith(",")) {
            daysSerialized = daysSerialized.substring(0, daysSerialized.length() - 1);
        }

        String[] daysArray = daysSerialized.split(",");
        Set<String> daysSet = new HashSet<>(Arrays.asList(daysArray));
        prefs.edit().putStringSet(PrefsActivity.PREF_COURSES_REMINDER_DAYS, daysSet).apply();
    }
